<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/user-portal.css">
    <title>OCC Lost and Found System</title>
</head>

<body>
    <nav class="navbar">
        <div class="left-side">
            <img src="../img/OCC-logo.webp" alt="">
            <h1>One Cainta College</h1>
        </div>
        <div class="right-side">
            <h1>User Portal</h1>
        </div>
    </nav>

    <div class="main-page">
        <img src="../img/occ-background.webp" alt="" class="occ-background">

        <div class="welcome-text">
            <h1>OCC LOST AND FOUND</h1>
            <h1>SYSTEM</h1>
            <h3>WELCOME USER!</h3>
        </div>

        <div class="search-container">
            <div class="search-bar">
                <input type="text" placeholder="Search items"><img src="../img/search-icon.png" alt="">
            </div>
            <a href="">Report a missing item?</a>
        </div>

        <div class="missing-items-container">
            <div class="missing-items-header">
                <div class="filled-header">
                    <h3 class="head-1">LOST ITEMS</h3>
                    <h3 class="head-2">DISCOVERY TIMESTAMP</h3>
                    <h3 class="head-3">FOUND BY</h3>
                    <h3 class="head-4">LAST SEEN LOCATION</h3>
                </div>

                <h3 class="space-provider"></h3>
            </div>

            <div class="all-items-container-column">
                <!-- first item -->
                <div class="all-items-container-row">
                    <div class="specific-item-container">
                        <div class="content-1"><img src="" alt=""></div>
                        <p class="content-2">2024/11/08 - 7:11 AM</p>
                        <p class="content-3">Melchor Abejuela</p>
                        <p class="content-4">Front of One Arena</p>
                    </div>

                    <div class="claim-message">
                        <button>Claim</button>
                        <img src="../img/message-icon.png" alt="" class="button-but-image">
                    </div>
                </div>

                <!-- second item -->
                <div class="all-items-container-row">
                    <div class="specific-item-container">
                        <div class="content-1"><img src="" alt=""></div>
                        <p class="content-2">2024/11/08 - 7:11 AM</p>
                        <p class="content-3">Melchor Abejuela</p>
                        <p class="content-4">Front of One Arena</p>
                    </div>

                    <div class="claim-message">
                        <button>Claim</button>
                        <img src="../img/message-icon.png" alt="" class="button-but-image">
                    </div>
                </div>

                <!-- third item -->
                <div class="all-items-container-row">
                    <div class="specific-item-container">
                        <div class="content-1"><img src="" alt=""></div>
                        <p class="content-2">2024/11/08 - 7:11 AM</p>
                        <p class="content-3">Melchor Abejuela</p>
                        <p class="content-4">Front of One Arena</p>
                    </div>

                    <div class="claim-message">
                        <button>Claim</button>
                        <img src="../img/message-icon.png" alt="" class="button-but-image">
                    </div>
                </div>

                <!-- fourth item -->
                <div class="all-items-container-row">
                    <div class="specific-item-container">
                        <div class="content-1"><img src="" alt=""></div>
                        <p class="content-2">2024/11/08 - 7:11 AM</p>
                        <p class="content-3">Melchor Abejuela</p>
                        <p class="content-4">Front of One Arena</p>
                    </div>

                    <div class="claim-message">
                        <button>Claim</button>
                        <img src="../img/message-icon.png" alt="" class="button-but-image">
                    </div>
                </div>

                <!-- fifth item -->
                <div class="all-items-container-row">
                    <div class="specific-item-container">
                        <div class="content-1"><img src="" alt=""></div>
                        <p class="content-2">2024/11/08 - 7:11 AM</p>
                        <p class="content-3">Melchor Abejuela</p>
                        <p class="content-4">Front of One Arena</p>
                    </div>

                    <div class="claim-message">
                        <button>Claim</button>
                        <img src="../img/message-icon.png" alt="" class="button-but-image">
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>

</html>